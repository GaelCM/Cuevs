...
import type { Producto, ProductoItem } from "@/types/Productos";
    ...
    persist<ListaProductosModel>(
        (set, get) => ({
            ...
            addProduct: (product: Producto) => {
                console.log("Agregando producto:", product.nombreProducto);
            ...
            getTotalPrice: () => {
                 const currentCarrito = get().carrito;
                 const total = currentCarrito.reduce((total, item) => total + (item.product.precio * item.quantity), 0);
